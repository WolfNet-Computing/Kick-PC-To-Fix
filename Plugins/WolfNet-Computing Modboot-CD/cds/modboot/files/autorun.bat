...
for %%i in (%cddrv%\level0\*.cab) do call %ramdrv%\bin\modboot.bat : _unpack %%i
for %%i in (%cddrv%\level1\*.cab %cddrv%\level2\*.cab %cddrv%\level3\*.cab) do call %ramdrv%\bin\modboot.bat : _unpack %%i -x
:_no0
rem if not exist %cddrv%\level1\*.cab goto _no1
rem for %%i in (%cddrv%\level1\*.cab) do call %ramdrv%\bin\modboot.bat : _unpack %%i -x
rem :_no1
rem if not exist %cddrv%\level2\*.cab goto _no2
rem for %%i in (%cddrv%\level2\*.cab) do call %ramdrv%\bin\modboot.bat : _unpack %%i -x
rem :_no2
rem if not exist %cddrv%\level3\*.cab goto _no3
rem for %%i in (%cddrv%\level3\*.cab) do call %ramdrv%\bin\modboot.bat : _unpack %%i -x
rem :_no3
